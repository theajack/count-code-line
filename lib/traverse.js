...
function handleFile ({base, filePath, name, config}) {
    ...
    // 排除非文本文件
    if (config.encodings.indexOf(encoding) === -1) {
        singleLineLog(filePath + ': ' + chalk.yellow(`(ignored encoding=${encoding})`));
        log();
        return null;
    }
    ...
    singleLineLog(filePath + ': ' + chalk.green(`[lines=${lines}]`) + ' ' + chalk.blue(`[characters=${characters}]`));
    log();
    return {
        name,
        type: 'file',
